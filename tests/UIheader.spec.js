...
test('test', async ({ page }) => {
  ...
  await page.goto('https://learn-staging.brightchamps.com/book-trial-class?courseId=1&fmt=51',{ waitUntil: 'domcontentloaded' });
  

  if (isMobile) {
    await page.waitForSelector('#topCompanyLogo', { state: 'visible', timeout: 30000 });
    await expect(page.locator('#topCompanyLogo')).toBeVisible();
    await page.locator('#topCompanyLogo').click();
  }
  await page.waitForSelector('text=Curriculum', { state: 'visible', timeout: 30000 });
  if(isDesktop){
...
await expect(page.getByText('TrustScore 4.6 3000+ reviews')).toBeVisible();
